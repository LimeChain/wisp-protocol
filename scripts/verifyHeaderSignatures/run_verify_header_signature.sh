...
run() {
  ...
  start=$(date +%s)
  node ../node_modules/snarkjs/cli.js groth16 prove "$TRUSTED_SETUP_DIR"/"$CIRCUIT_NAME".zkey "$COMPILED_DIR"/"$CIRCUIT_NAME"_cpp/witness.wtns "$SLOT_PROOF"/proof.json "$SLOT_PROOF"/public.json
  end=$(date +%s)
  echo "DONE ($((end - start))s)"

  echo "====VERIFYING PROOF FOR GIVEN SLOT===="
  start=$(date +%s)
  node ../node_modules/snarkjs/cli.js groth16 verify "$TRUSTED_SETUP_DIR"/vkey.json ./"$SLOT_PROOF"/input.json "$SLOT_PROOF"/proof.json
  end=$(date +%s)
  echo "DONE ($((end - start))s)"
