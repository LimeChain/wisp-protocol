...
BASE_CIRCUIT_DIR=../../circuits/verify_ssz_to_poseidon_commitment
...
run() {
  ...
  start=$(date +%s)
  yarn ts-node --project tsconfig.json ./syncCommitteeComittment/generateProofInput.ts --period ${SYNC_COMMITTEE_PERIOD}
  end=$(date +%s)
